from meetup.models import Meetup, Meetup, Event, Attendee, Attendance
...
def sync_meetup(name: str) -> None:
    ...
    try:
        meetup_instance = Meetup.objects.get(name=name)
    ...
    for event in events:
        ...
        event_instance, created = Event.objects.get_or_create(
            event_id=event_id, meetup_id=meetup_instance.pk, defaults=defaults
        )
        ...
        attendance = fetch_attendance(
            meetup_instance.name, event_instance.event_id, api_key
        )
        for attendee in attendance:
            ...
            attendee_instance, created = Attendee.objects.get_or_create(
                member_id=member_id, defaults=defaults
            )
