...
class RootApiView(rest_framework.views.APIView):
	def get(self, request: rest_framework.request.Request) -> rest_framework.response.Response:
		routes = {
			...
			'attendees': rest_framework.reverse.reverse('attendee-list', request=request),
			'attendance': rest_framework.reverse.reverse('attendance-list', request=request),
			'graphql-drf': rest_framework.reverse.reverse('graphql-drf', request=request)
		}
